...
void CHybrid::get_positions_from_analytical_formula(INT32 id_line, INT32 &num_positions, D_REAL** &positions)
{
	...
	D_REAL rperi = R_Moon+500.e+03/SI_x0;
	D_REAL rapo = R_Moon+15000.e+03/SI_x0;
	
	D_REAL a = 1./2.*(rperi + rapo );
	D_REAL e = 1./2.*(rapo  -rperi);
	D_REAL b = sqrt(rapo*rperi);
	
	//! NOTE NOTE NOTE orbits are parametrized by angle phi, which is defined with respect to center of
	//! NOTE NOTE NOTE ellipsis NOT with respect to the focal point of the ellipsis (i.e. not with respect
	//! NOTE NOTE NOTE to Mercury's center)
	
	//! NOTE NOTE NOTE this angle phi is calculated in paralell_to_single_file function below
	
	//! NOTE NOTE NOTE for ENERGY_SPECTRUM, the parallel_output_to_single_lineout_file_energy_spectrum
	//! NOTE NOTE NOTE function is in CHybrid_EnergySpectrum.cpp
...
// 	delta_r[2] = LZ/2.;
	...
	num_positions = int(M_PI*(a+b)/(0.5*delta_r[0]));
...
// 		positions[2][counter] = start_pos[2] + line_vec[2]*delta_r[2]*counter;
		...
		delta_phi += 2.*M_PI/(1.*num_positions);
		
		//! --- Ellipsis at y = 0, MPO
		if(id_line==0)
		{		
		  	D_REAL rperi_0 = 2840.e+03/SI_x0;
			D_REAL rapo_0 = 3948.e+03/SI_x0;
			
			D_REAL a_0 = 1./2.*(rperi_0 + rapo_0 );
			D_REAL e_0 = 1./2.*(rapo_0  -rperi_0);
			D_REAL b_0 = sqrt(rapo_0*rperi_0);
			
			
			positions[0][counter] = -a_0*sin(delta_phi) + e_0;
			positions[1][counter] = 0;
			positions[2][counter] = b_0*cos(delta_phi);
		}
		
		//! --- Ellipsis at y = 0, MMO
		if(id_line==1)
		{		
		  D_REAL rperi_1 = 2840.e+03/SI_x0;
		  D_REAL rapo_1 = 14264.e+03/SI_x0;
		  
		  D_REAL a_1 = 1./2.*(rperi_1 + rapo_1 );
		  D_REAL e_1 = 1./2.*(rapo_1  -rperi_1);
		  D_REAL b_1 = sqrt(rapo_1*rperi_1);
		  
			positions[0][counter] = -a_1*sin(delta_phi) + e_1;
			positions[1][counter] = 0;
			positions[2][counter] = b_1*cos(delta_phi);
		}
				//! --- Ellipsis at x = 0
		if(id_line==2)
		{		
			positions[0][counter] = 0;
			positions[1][counter] = b*cos(delta_phi);
			positions[2][counter] = a*sin(delta_phi) - e;
		}
		//! ----------------------------------------------------------
		//! ----------------------------------------------------------
...
void CHybrid::parallel_output_to_single_file_lineout(INT32 id_line, INT32 id_field)
{
	...
	for(int process=0; process<mpi_num_processes; process++)
	{
		...
		for(;;)
		{
			...
			if(id_line == 0)
			{	
				phi = (atan2(r[0],r[2]+7250./SI_x0) + M_PI)*360./(2.*M_PI);
			}
			
			if(id_line == 1)
			{	
				phi = (atan2(r[1],r[2]+7250./SI_x0) + M_PI)*360./(2.*M_PI);
			}
	...
	for(INT32 counter=0; counter<num_positions; counter++)
	{
		...
		if(calc_BlockNodeXCell_of_Pos(index_blk, index_cell, x_cell, r_vec))
		{
			...
			if(mpi_myRank == top_level_Block->responsible_mpi_process)
			{
				...
				lineout_outfile << counter <<"		"
						<< positions[0][counter] *LX -Box_Origin[0] <<"		"

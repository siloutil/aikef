...
void CBlock::fill_empty_Volume(INT32 species, const INT32* start, const INT32* end)
{
		...
		if(!TL_change_dens_start || TL <  TL_change_dens_start)
		rho_init = rho_sw[species];
		else
		{
			rho_init = rho_sw[species];

			if(TL>=TL_change_dens_start && TL<TL_change_dens_start+Num_TL_change_dens)
			rho_init = (rho_sw_final[species]-rho_sw[species])/Num_TL_change_dens
					*(TL-TL_change_dens_start) + rho_sw[species];
			else
			rho_init = rho_sw_final[species];					
		}	

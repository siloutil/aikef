	...
	if(!Neighbour[_Im1_] && !is_box_boundary[_Im1_])
	{
		...
		if(parent->responsible_mpi_process != mpi_myRank)
		{
		  ...
		  //req_is_MPI_send_completed[PART_request].Wait();
                  MPI_Wait(&req_is_MPI_send_completed[INFO_request], MPI_STATUS_IGNORE); // ND 05/04/18
                  MPI_Wait(&req_is_MPI_send_completed[PART_request], MPI_STATUS_IGNORE); // ND 05/04/18
		}
	}
...
void CBlock::wait_for_completedSendParticle_MPI(INT32 _m1_, INT32 _p1_)
{
	...
	if(_m1_>=0 && Neighbour[_m1_] && mpi_myRank!=Neighbour[_m1_]->responsible_mpi_process)
	{
	  ...
	  //req_is_MPI_send_completed[id_REQ_PART_m1].Wait();
          MPI_Wait(&req_is_MPI_send_completed[id_REQ_INFO_m1], MPI_STATUS_IGNORE); // ND 05/04/18
          MPI_Wait(&req_is_MPI_send_completed[id_REQ_PART_m1], MPI_STATUS_IGNORE); // ND 05/04/18
	}
	...
	if(Neighbour[_p1_] && mpi_myRank!=Neighbour[_p1_]->responsible_mpi_process)
	{
	  ...
	  //req_is_MPI_send_completed[id_REQ_PART_p1].Wait();
          MPI_Wait(&req_is_MPI_send_completed[id_REQ_INFO_p1], MPI_STATUS_IGNORE); // ND 05/04/18
          MPI_Wait(&req_is_MPI_send_completed[id_REQ_PART_p1], MPI_STATUS_IGNORE); // ND 05/04/18
	}

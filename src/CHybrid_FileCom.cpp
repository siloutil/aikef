...
void CHybrid::add_fields_to_average_fields(void)
{
	...
	{	
		...
		if(IDs_of_Fields_to_average[average_field]==ION_DENSITY)
		{
			set_zero_field(ION_DENSITY);
			copy_Field(ION_DENSITY,id_rho_np1);
			add_multipliedField(ION_DENSITY,id_rhoSpecies1 +5,-1);		
		}
		
		if(IDs_of_Fields_to_average[average_field]==ION_VELOCITY)
		{
			set_zero_field(ION_VELOCITY);			
			
			copy_Field(ION_VELOCITY,id_UI_plus);
		}
	}
	...
	for(INT32 flyby=0; flyby<num_trajectories; flyby++)
	  ...
	  {	
		...
		if(TL)
		{
			...
			if(flyby==0 || flyby==1 || flyby==2 || flyby==7 || flyby==15 || flyby==17) // trajectory ungereimtheit
			trace_field_flyby[flyby][field]=true;
		}
		else
		{
...
// 		   FULL_GN_UPDATE(id_scratch_vector);
	...
	if(num_ion_prod_fields>0)
	for(INT32 ionProdField=0; ionProdField<num_ion_prod_fields; ionProdField++)
	{
		silo_3DWrite_Field(id_density_ionProdSpecies1+ionProdField);
		silo_3DWrite_Field(id_velocity_ionProdSpecies1+ionProdField);
	}
#endif

...
void CBlock::init_Eta_Profile(void)
{
	...
	for(INT32 u=0; u< BlkNds_X; u++)
	  for(INT32 v=0; v< BlkNds_Y; v++)
		...
		if(dist_to_orig < obstacle_core_fraction* R_Moon)
                    Eta[u_v_w] = 0.;
		
		//! outer core for smoother eta gradient
		if(obstacle_core_fraction* R_Moon < dist_to_orig)
		  if(dist_to_orig < 0.75*R_Moon)
		Eta[u_v_w] = 0.;

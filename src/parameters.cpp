...
const short flyby = 14; 
//! electron impact ionization frequency in 1/s
const D_REAL Ence_nu_e_total = 3e-08; 
...
const D_REAL average_ion_mass = 1;
...
const D_REAL neutral_torus_density = 1.e+05*1e+06/SI_n0;
//! ---------------------------------------------------------------------------


//! Normalisation quantities
//! Magnetic Field in Telsa
const D_REAL SI_B0 = 22.32e-9;
//! Number Density in Particle per Cubicmeter
const D_REAL SI_n0 = 10.e6;
...
const D_REAL Parker_angle = 0;
//const D_REAL B_sw[3] = {cos(Parker_angle*M_PI/180.)*cos(B_angle*M_PI/180.), -sin(Parker_angle*M_PI/180.)*cos(B_angle*M_PI/180.), +sin(B_angle*M_PI/180.)};
const D_REAL B_sw[3] ={0.09 , -0.90 , -0.426};

//! Velocity of Solar Wind / upstream plasma
//! in Metre Per Second;
const D_REAL SW_v = 500e+3;
...
// const D_REAL V_sw[3] = {8.2, 0., 0.};

//! upstream density at begin of simulation
const D_REAL n0_start = 140.e6;
...
//! ------------------------------------------------------------------
const INT32 BlkNds_X = 12;
const INT32 BlkNds_Y = 12;
const INT32 BlkNds_Z = 12;
...
const D_REAL R_Moon = 2440.e+3/SI_x0;

//! Size of simulation box
const D_REAL LX = 12.*R_Moon;
const D_REAL LY = 12.*R_Moon;
const D_REAL LZ = 12.*R_Moon;

//! Origin of simulationbox (= Position of Obstacle)
const D_REAL Box_Origin[3] = {0.3501 * LX, 0.5001 * LY, 0.5001 * LZ}; //x was 0.3501*R_Moon accidentially
...
const INT32 resistive_bound_cells[]  = {0,5,
                                        5,5,
                                        5,5};

//! specify value of eta at boundary
const F_REAL resistive_bound_eta[]  = { 0., 1.,
                                        1., 1.,
                                        1., 1.};
...
const char Run_Name[RUN_NAME_SIZE] = "Mercury_konvergenz_lvl2_0";
...
const INT32  TL_OUTPUT_3D_SILO	 =     20;
...
const INT32 TL_SAVE_STATE  = 4000;
...
const INT32 resubmit_walltime =11*3600;
const char resubmit_jobscript[100] = "msub job.sh";
const INT32 resubmit_security_factor = 100;
...
const char visit_length_unit[20] = "RM";
...
//! specify names of files in which positions of trajectories are stored
//! since these names are also used as variable names for
//! silo ouput, only alphanumeric characters are allowed
//! (especially no "." )
const char Trajectory_FileName[][RUN_NAME_SIZE] = {"orbit_mes_cme2",
						"orbit_mmo_xz",
						"orbit_mpo_xz"
						"orbit_mes_cme1"};
...
const INT32 num_fields_to_trace = 6 + 4;
...
//! silo ouput, only alphanumeric characters are allowed
//! (especially no "." )
const char LineOut_FileName[][RUN_NAME_SIZE] = { "lineout_orbit_mpo",
						 "lineout_orbit_mmo",
						 "lineZ"};


//! Number of fields in Lineout Output                                              
const INT32 num_fields_to_trace_lineout = 9;
...
const INT32 num_average_fields = 5;
...
const INT32  num_TL_average_Fields = TL_OUTPUT_3D_SILO-1;
...
const INT32 IDs_of_Fields_to_average[] = {id_rotB,id_UI_plus,id_BEven, id_BTotal, id_rho_np1};
...
const INT32 uniform_grid_num_fields_to_trace = 4;

//! id's of fields which should appear in uniform_grid visualization
const INT32 uniform_grid_ID_of_Fields_to_trace[] = {id_average_Field1,
						id_average_Field1+1,
						id_average_Field1+2,
						id_average_Field1+3,
						id_rho_np1,id_UI_plus, id_BTotal,
					      id_EField,
					      };
...
//! for 1/cm3
const INT32 uniform_grid_Field_type[] = {uniform_grid_TYPE_DENSITY_FIELD,
...
const char Detector_FileName[][RUN_NAME_SIZE] = { "northpole",
                                                    "dayside",
                                                    "nightside"};

//! Detector Box Geometry

const D_REAL detector_box_xmin[] = {-.5*R_Moon,-2.0*R_Moon,1.0*R_Moon,-100000./SI_x0,0};
const D_REAL detector_box_xmax[] = {.5*R_Moon,-0.7*R_Moon,3.0*R_Moon,-10000./SI_x0,0};
const D_REAL detector_box_ymin[] = {-.5*R_Moon,-0.5*R_Moon,-0.5*R_Moon,-10000./SI_x0,0};
const D_REAL detector_box_ymax[] = {.5*R_Moon,.5*R_Moon,.5*R_Moon,10000./SI_x0,0};
const D_REAL detector_box_zmin[] = {1.0*R_Moon,-.3*R_Moon,-.3*R_Moon,-10000./SI_x0,0};
const D_REAL detector_box_zmax[] = {1.5*R_Moon,.3*R_Moon,.3*R_Moon,10000./SI_x0,0};
...
const D_REAL smooth_Factor = 0.002;
const D_REAL smooth_E[] = { smooth_Factor, smooth_Factor * 1.5, smooth_Factor *2.25 , smooth_Factor * 8.};
const D_REAL smooth_B[] = { smooth_Factor, smooth_Factor * 1.5, smooth_Factor *2.25 , smooth_Factor * 8.};
...
const INT32  num_smooth_eta = 15;
const D_REAL smooth_eta[] = {1./8. *0.25,  1./4. *0.25, 1./2. *0.25, 1. * 0.5,  1.* 0.5};

#if defined(nonadiabatic_gradPE_TERM_smooth_PE)
const D_REAL smooth_PE[] = { smooth_Factor * 1., smooth_Factor * 1.5, smooth_Factor *2.25 , smooth_Factor * 8.};
#endif
...
const int num_merge_each_cell = 100;
const int num_split_each_cell = 50;
...
const bool do_split_in_species[] = {true, false, true, true, true, true};
const bool do_merge_in_species[] = {true, false, true, true, true, true};
//! --------------------------------------------------------------
...
//! "left, lower corner"
const D_REAL minX_refBox_of_L[] = {-7.*R_Moon, -2.*R_Moon, -2.*R_Moon};
const D_REAL minY_refBox_of_L[] = {-7*R_Moon, -2.*R_Moon, -2.*R_Moon};
const D_REAL minZ_refBox_of_L[] = {-5.*R_Moon, -2.*R_Moon, -5.*R_Moon};
//! "right, upper corner"
const D_REAL maxX_refBox_of_L[] = { 7.*R_Moon, 2.*R_Moon, 2.*R_Moon};
const D_REAL maxY_refBox_of_L[] = { 7*R_Moon, 2.*R_Moon, 2.*R_Moon};
const D_REAL maxZ_refBox_of_L[] = { 5.*R_Moon, 2.*R_Moon, 2.*R_Moon};
...
const INT32 TL_REFINE_STATIC_SPHERE[] = {25, 55, -1, -1, -1};


const D_REAL radius_refSphere_of_L[] = { 4.*R_Moon, 3.5*R_Moon, 30., 0., 0.};


//! C) Zylinder along x
const INT32 TL_REFINE_STATIC_ZYLINDER_X[] = {-1, -1, -1, -1, -1};

const D_REAL radiusX_refZylinder_of_L[] = {5.* R_Moon, 2.5* R_Obstacle};

const D_REAL minX_refZylinder_of_L[] = { -3.*R_Moon, -LX};
const D_REAL maxX_refZylinder_of_L[] = { +6.*R_Moon,  0.};
...
const D_REAL radiusZ_refZylinder_of_L[] = { 5.5*R_Moon, 3.5*R_Moon, 1.5*R_Moon, 2.*R_Obstacle};

const D_REAL minZ_refZylinder_of_L[] = {-7*R_Moon, -3*R_Moon, -7.*R_Moon, -1.*R_Obstacle};
const D_REAL maxZ_refZylinder_of_L[] = {7*R_Moon,  3*R_Moon, 2.5*R_Moon, +1.*R_Obstacle};
...
const INT32 TL_REDISTRIBUTE_BLOCKS = 10000;
...
const INT32 optimal_MPiC[] = {5, 5, 5, 5, 5, 0};
...
const D_REAL min_particle_weight = 1e-6;
...
const D_REAL rho_sw[] = {1., 0.,  0.,  0.,  0., 0.};

//! change upstream density during simulation for different MA
//! TL to start change in density
//! set to zero to switch it off
const INT32 TL_change_dens_start = 0;
//! number of time steps for linear change in density
const INT32 Num_TL_change_dens = 500;
//! final density
const D_REAL rho_sw_final[] = {1. , 0.,  0.,  0.,  0., 0.};
...
const D_REAL Ti_para[] = {kB*25.e+5,kB*25.e+5,7*e,7*e,1.1*e,0};
const D_REAL Ti_perp[] = {kB*25.e+5,kB*25.e+5,35*e,35*e,2.2*e,0};


//!electron temperature in K
const D_REAL Te = 25.e+5;      
//! electron plasma betas for each Species.
const D_REAL Electron_Betas[]  = {calcBeta*Te, calcBeta*Te, 0., 0.};
// const D_REAL Electron_Betas[]  = {0.69, 0.69, 0., 0.};
...
const D_REAL kappa_electron =  2.0;
	...
	const bool neutral_profile_from_file = true;
...
// 	const D_REAL Neutral_Masses[] = { 17.885, 1., 1., 1. };
	...
	//! Beta of newly ionised electrons
        const D_REAL NewElectron_Betas[]  = {calcBeta*180., 0*calcBeta*180., calcBeta*180., calcBeta*180.};
// 	const D_REAL NewElectron_Betas[]  = {0.*calcBeta*100., 0*calcBeta*180., calcBeta*180., calcBeta*180.};
	...
	const D_REAL norm_externNeutralDensity[] = {4.9,1,1};

	const D_REAL norm_externNeutralVelocity[] = {1.e-03,1,1};

	//! file names for reading extern fields
	const char extern_NeutralField_name[][80] = { "../../../Neutral_E7_SourceCorr.dat",
							"lineY",
							"lineZ"};
...
const D_REAL en_coll_param[] = {                                                                                                                          
	...
	const INT32 num_Dust_Species = 1;
	...
	//! file names for reading extern fields
	const char extern_DustField_name[][80] = { "../../../Staub_nu3E14_J_M05_CCC_3.dat",
							"lineY",
							"lineZ"};
							
	//! Full dust density at TL
	const INT32 TL_DUST_MAX = 40000;//40000*(1.-0.5*LOWRES)*(1.- 49./50.*VOYAGER);
	...
	//! file names for reading extern fields
	const char extern_IonProdField_name[][80]= { "../../../Staub_nu3E14_J_M05_CCC_3.dat",
							"lineY",
							"lineZ"};
...
const D_REAL B_SOR_omega_L[] = {1.01,1.15,1.5,1.45};
//{1.2, 1.3, 1.35, 1.4};

//! OM3) choose break criteria for SOR
const D_REAL B_SOR_max_error = 1.e-5;
...
const INT32 B_SOR_calc_error_step = 2;
...
const D_REAL DC_omega_L[] = {1.4, 1.52, 1.75, 1.92}; 

//! DC3) choose break criteria for SOR
const D_REAL DC_max_error = 5.e-5;

//! DC4) B_SOR_num_cycle_base^L cycles will be performed
//!      in level L each iterations
const INT32 DC_num_cycle_base = 4;

//! DC5) choose maximal number of iterations for SOR
const INT32 DC_max_Iteration = 4000;
...
const INT32 num_optimize_SOR_steps = 20;
...
const D_REAL obstacle_core_fraction = 0.25;
...
const D_REAL Magnetic_Moment[3] = { 0., 0.,  -200.e-9/SI_B0*R_Moon*R_Moon*R_Moon};

//! Obstacle's intrinsic Quadrupole Field
//! MM is defined such that at the origin MM and BField are parallel.
const D_REAL Magnetic_Moment_Quad[3] = { 0., 0.,  -45.e-12/SI_B0*R_Moon*R_Moon*R_Moon};
...
const D_REAL Magnetic_Moment_Offset[3] = {0., 0., +480.e+3/SI_x0};

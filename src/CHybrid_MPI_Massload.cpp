		    ...
		    && new_responsible_mpi_process[blk] != mpi_myRank)
		{
			...
			if(sync_mpi_send_rec) {
			  ...
			  //BlkList[blk]->req_is_MPI_send_completed[id_request_Particle].Wait();
                          MPI_Wait(&BlkList[blk]->req_is_MPI_send_completed[id_request_Fields], MPI_STATUS_IGNORE); // ND 05/04/18
                          MPI_Wait(&BlkList[blk]->req_is_MPI_send_completed[id_request_MPiCInfo], MPI_STATUS_IGNORE); // ND 05/04/18
                          MPI_Wait(&BlkList[blk]->req_is_MPI_send_completed[id_request_Particle], MPI_STATUS_IGNORE); // ND 05/04/18
			}
			else {
